package com.duoker.watch.db;

import java.io.Serializable;

/**
 * Created by chengang on 4/24/2017.
 */

public class WatchRecord implements Serializable {
    private String addr;
    private String altitude;
    private String calcTime;
    private String cellID;
    private String devdistance;
    private String devname;
    private String devsex;
    private String directionOffSet;
    private String directionSpeed;
    private String electricity;
    private String gpsQuality;
    private long groupId;
    private String headImage;
    private String lac;
    private String latitude;
    private String longitude;
    private String mcc;
    private String mnc;
    private int online;
    private String owner;
    private String phoneIMS;
    private String projectCate;
    private String rss;
    private String watchId;

    public String getAddr() {
        return this.addr;
    }

    public String getAltitude() {
        return this.altitude;
    }

    public String getCalcTime() {
        return this.calcTime;
    }

    public String getCellID() {
        return this.cellID;
    }

    public String getDevdistance() {
        return this.devdistance;
    }

    public String getDevname() {
        return this.devname;
    }

    public String getDevsex() {
        return this.devsex;
    }

    public String getDirectionOffSet() {
        return this.directionOffSet;
    }

    public String getDirectionSpeed() {
        return this.directionSpeed;
    }

    public String getElectricity() {
        return this.electricity;
    }

    public String getGpsQuality() {
        return this.gpsQuality;
    }

    public long getGroupId() {
        return this.groupId;
    }

    public String getHeadImage() {
        return this.headImage;
    }

    public String getLac() {
        return this.lac;
    }

    public String getLatitude() {
        return this.latitude;
    }

    public String getLongitude() {
        return this.longitude;
    }

    public String getMcc() {
        return this.mcc;
    }

    public String getMnc() {
        return this.mnc;
    }

    public int getOnline() {
        return this.online;
    }

    public String getOwner() {
        return this.owner;
    }

    public String getPhoneIMS() {
        return this.phoneIMS;
    }

    public String getProjectCate() {
        return this.projectCate;
    }

    public String getRss() {
        return this.rss;
    }

    public String getWatchId() {
        return this.watchId;
    }

    public void setAddr(String paramString) {
        this.addr = paramString;
    }

    public void setAltitude(String paramString) {
        this.altitude = paramString;
    }

    public void setCalcTime(String paramString) {
        this.calcTime = paramString;
    }

    public void setCellID(String paramString) {
        this.cellID = paramString;
    }

    public void setDevdistance(String paramString) {
        this.devdistance = paramString;
    }

    public void setDevname(String paramString) {
        this.devname = paramString;
    }

    public void setDevsex(String paramString) {
        this.devsex = paramString;
    }

    public void setDirectionOffSet(String paramString) {
        this.directionOffSet = paramString;
    }

    public void setDirectionSpeed(String paramString) {
        this.directionSpeed = paramString;
    }

    public void setElectricity(String paramString) {
        this.electricity = paramString;
    }

    public void setGpsQuality(String paramString) {
        this.gpsQuality = paramString;
    }

    public void setGroupId(long paramLong) {
        this.groupId = paramLong;
    }

    public void setHeadImage(String paramString) {
        this.headImage = paramString;
    }

    public void setLac(String paramString) {
        this.lac = paramString;
    }

    public void setLatitude(String paramString) {
        this.latitude = paramString;
    }

    public void setLongitude(String paramString) {
        this.longitude = paramString;
    }

    public void setMcc(String paramString) {
        this.mcc = paramString;
    }

    public void setMnc(String paramString) {
        this.mnc = paramString;
    }

    public void setOnline(int paramInt) {
        this.online = paramInt;
    }

    public void setOwner(String paramString) {
        this.owner = paramString;
    }

    public void setPhoneIMS(String paramString) {
        this.phoneIMS = paramString;
    }

    public void setProjectCate(String paramString) {
        this.projectCate = paramString;
    }

    public void setRss(String paramString) {
        this.rss = paramString;
    }

    public void setWatchId(String paramString) {
        this.watchId = paramString;
    }
}
