...
public class HeartRateDayViewComp extends BaseComponent {
    public static final String TAG = HeartRateDayViewComp.class.getSimpleName();
    private TextView mAverageView;
    private LineChart mChart;
    private List<HeartRateBean> mData;
    private TextView mGreatView;
    private HeartRateDayPresenter mHeartRatePresenter;
    private TextView mLowView;
    private DateRecyclerView mRecyclerView;

    public HeartRateDayViewComp(Activity paramActivity, HeartRateDayPresenter paramHeartRateDayPresenter) {
        super(paramActivity);
        setContentView(R.layout.comp_heart_rate_month_view);
        this.mHeartRatePresenter = paramHeartRateDayPresenter;
        initView();
        initListener();
        initChart();
        initData();
    ...
    private LineData getLineData(List<HeartRateBean> inList) {
        if (inList == null)
            return null;
        ...
        for (int j = 0; j < size; j++) {
            int heartRate
                    = inList.get(j).getHeartRate();
            tmp.add(new Entry(j, heartRate));
        }
        this.mChart.getXAxis().setLabelCount(size, true);

        LineDataSet lineDataSet = createSet(tmp);
        ArrayList localArrayList2 = new ArrayList();
        localArrayList2.add(lineDataSet);
    ...
    private void initListener() {
        this.mRecyclerView.setOnItemClickListener(new DateRecyclerView.OnItemClickListener() {
            public void onItemClick(DateHelper.DateItem paramAnonymousDateItem, int paramAnonymousInt) {
                Log.d(HeartRateDayViewComp.TAG, "position--" + paramAnonymousInt);
                Date localDate = paramAnonymousDateItem.getDate();
                Log.d(HeartRateDayViewComp.TAG, "date--" + localDate.toString());
                HeartRateDayViewComp.this.mHeartRatePresenter.setDate(localDate);
                HeartRateDayViewComp.this.mHeartRatePresenter.getHeartRate();
            }
        });
    }
    ...
    private void initView() {
        ...
        this.mRecyclerView.setType(PickerTypeEnum.DAY);
    ...
    public class XAxisValueFormatter implements IAxisValueFormatter {
        private String TIME_FORMAL = "HH:mm:ss";
        ...
        public String getFormattedValue(float paramFloat, AxisBase paramAxisBase) {
            ...
            try {
                String str = DateFormatUtils.format(1000L * ((HeartRateBean) HeartRateDayViewComp.this.mData.get(i)).getCalcTime(), this.TIME_FORMAL);
                return str;
